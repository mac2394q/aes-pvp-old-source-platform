...
<?php
		if($_SESSION["user"]["role"] == SA || $_SESSION["user"]["role"] == ADMIN || $_SESSION["user"]["role"] == COORDINADOR ):
?>
							<form action="<?=SITE_URL?>company/providers/<?=$company["id"]?>" method="post" style="text-align:center;">
...
?>	
					<tr>
						<td colspan="3">No existen proveedores asociados para <?=$company["socialName"]?></td>
					</tr>
<?php 
		} 
?>	
				...
				<p class="search">
					<form action="<?=SITE_URL?>company/providers/<?=$company["id"]?>" method="post" style="text-align:center;">
						<label for="search"><strong>Buscar Proveedores para Agregar</strong></label>
...
?>
						<tr>
							...
							<td><?=$companie["sectorName"]?></td>
							<td>
								<form action="<?=SITE_URL?>company/providers/<?=$company["id"]?>" method="post">
									<input type="hidden" name="search" value="<?=isset($search) ? $search: "";?>">
									<input type="hidden" name="provider" value="<?=$companie["id"]?>">
